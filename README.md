# kgrid36
36% (4x10 → 最下段6キー) Grid Layout 一体型カスタムキーボードプロジェクト。

## 1. プロジェクトの経緯
本リポジトリは kgrid40 をベースに、最下段両端2キーずつ（計4キー）を削除した36キー構成への変更を実施。
名称の完全統一と物理基板（PCB）との整合性確認を完了。

**実機（SuperMini nRF52840）への書き込み、Bluetooth接続、および全36キーのマトリックス導通テストを完了し、ハードウェア・ソフトウェアの両面で動作を実証。**
...
### `config/kgrid40.json`
- layoutエントリを40→36に削減。row3は col2〜col7 の6キーのみ。

### `config/kgrid40.keymap`
- 全レイヤーのバインディングを40スロット→**36スロット**に変更。
- row3の割り当て：`LALT / LGUI / SPACE / SPACE / ENTER / BSPC`。
...
### 2026-05-04: kgrid40→kgrid36への変更
* **36キー構成への変更**: row3両端2キーずつ（RC(3,0)(3,1)と RC(3,8)(3,9)）を削除。
* **dtsi修正**: matrix_transformのmapを40→36キーに変更。
* **json修正**: layoutエントリを36に削減。
* **keymap修正**: 全3レイヤーのバインディングを36スロットに変更。
* **Keymap Editor確認**: 36キーレイアウトの正常表示を確認。
...
* **キーマップ変更**: `.keymap` を修正してPush → 自動ビルド → `kgrid40.uf2` を書き込む。全**36スロット**分を編集すること。
